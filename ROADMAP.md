...
| **3.1. VS Code + GitHub Copilot Chat detection** | In-process JavaScript chat: scoped activity probe under `~/Library/Application Support/Code{,- Insiders}/User/workspaceStorage/*/chatEditingSessions/`, gated by a VS Code host-process check. Copilot CLI sessions are already covered by the `copilot` process + `${COPILOT_HOME:-~/.copilot}/session-state` probe. | planned — [`future/phase-3.1-vscode-copilot-chat.md`](./future/phase-3.1-vscode-copilot-chat.md) |
