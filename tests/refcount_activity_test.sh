...
#
# CRITICAL: this helper does NOT echo its path. Callers must use
# $VIGIL_ACTIVE_DIR directly. If we ran the body inside $(...)
# command substitution and captured stdout, the `export` would happen
# in a subshell and never propagate to the test body — which would
# silently make vigil_refcount_count/list read the user's REAL state
# dir (~/Library/Application Support/vigil/state/active). When the
# vigil daemon is running, that real dir contains live cli-claude-*.pid
# entries, and the assertions in this file would either pass by
# accident or fail nondeterministically depending on what the daemon
# was tracking at the moment the test ran. The pre-phase-3 version of
# this file had exactly that flake; do not reintroduce it.
_setup_active_dir() {
...
test_count_with_claude_active_only() {
    ...
    _make_pidfile "$VIGIL_ACTIVE_DIR" "cli-claude-1002"
    _make_pidfile "$VIGIL_ACTIVE_DIR" "cli-codex-1003"
    _make_pidfile "$VIGIL_ACTIVE_DIR" "wrapper-1004"
...
test_count_with_app_codex_gated_on_codex_flag() {
    ...
    _setup_active_dir
    _make_pidfile "$VIGIL_ACTIVE_DIR" "app-codex-2700"
    _make_pidfile "$VIGIL_ACTIVE_DIR" "cli-claude-1001"
...
test_list_state_column_matches_flags() {
    _setup_active_dir
    _make_pidfile "$VIGIL_ACTIVE_DIR" "cli-claude-1001"
    _make_pidfile "$VIGIL_ACTIVE_DIR" "cli-codex-1002"
    _make_pidfile "$VIGIL_ACTIVE_DIR" "app-codex-2700"
    _make_pidfile "$VIGIL_ACTIVE_DIR" "wrapper-1003"
...
test_filename_parser_handles_all_prefixes() {
    _setup_active_dir
    _make_pidfile "$VIGIL_ACTIVE_DIR" "cli-claude-1"
...
test_wrappers_count_regardless_of_agent_flags() {
    _setup_active_dir
    _make_pidfile "$VIGIL_ACTIVE_DIR" "wrapper-1234"
