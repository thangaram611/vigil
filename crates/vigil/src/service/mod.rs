...
/// systemd user-unit name for the resident daemon on Linux.
pub const SYSTEMD_USER_UNIT_NAME: &str = "com.thangaram.vigil.service";
/// Linux logrotate drop-in path for the daemon log.
pub const LINUX_LOGROTATE_FILE: &str = "/etc/logrotate.d/vigil";
...
const LINUX_AGENT_PATH_ENV: &str = "/usr/local/bin:/usr/local/sbin:/usr/bin:/usr/sbin:/bin:/sbin";
...
/// `$HOME/.config/systemd/user/com.thangaram.vigil.service`.
pub fn systemd_user_unit_path() -> std::path::PathBuf {
...
/// Render the Linux systemd user unit for the resident daemon.
pub fn render_systemd_user_unit(cfg: &VigilConfig) -> String {
    let exe = format!("{}/bin/vigil", cfg.install_dir);
    format!(
        "[Unit]\n\
         Description=Vigil agent activity sleep guard\n\
         Documentation=https://github.com/thangaram/vigil\n\
         ...
         Restart=always\n\
         RestartSec=10\n\
         Environment=\"PATH={}\" \"VIGIL_STATE_DIR={}\" \"VIGIL_LOG_DIR={}\"\n\
         \n\
         [Install]\n\
         WantedBy=default.target\n",
        systemd_escape(&exe),
        systemd_escape(LINUX_AGENT_PATH_ENV),
        systemd_escape(&cfg.state_dir),
        systemd_escape(&cfg.log_dir),
    )
}

/// Render the Linux logrotate drop-in for the daemon log.
pub fn render_linux_logrotate(cfg: &VigilConfig) -> String {
    format!(
        "# {LINUX_LOGROTATE_FILE} - generated by `vigil setup`\n\
         {}\n\
         {{\n\
         \tsize 1M\n\
         \trotate 5\n\
